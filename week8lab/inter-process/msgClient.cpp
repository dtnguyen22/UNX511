...
int main(void)
{
    ...
    fd=socket(AF_INET, SOCK_DGRAM, 0);//socket that handles connections
    if(fd<0) {
    ...
    ret = inet_pton(AF_INET, IP_ADDR, &servaddr.sin_addr);//convert presentation  to network binary format and store it to server socket
    if(ret==0) {
