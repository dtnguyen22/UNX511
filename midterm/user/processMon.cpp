...
int main()
{
    ...
    //answer: processMonitor: SIGINT received
    ...
    //answer: key_generator_open() (int file_operations simple_driver_fops)
    ...
    for (int i = 0; i < NUM; ++i)
    {
        ...
        //answer: static void sigHandler(int sig);
...
int processMonitor()
{
    ...
    while (isRunning && pid >= 0)
    {
        ...
        //answer: child process terminated and returns its pid
        pid = wait(&status);
        char buf[MAXBUF];
        //Question 10: What is processMon attempting to do with the log file?
        int openFlags = O_RDWR | O_CREAT | O_APPEND; //read write, append
        mode_t filePerms = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;//666
        int fdLog = open(logFile, openFlags, filePerms);//open log file
        int flags = fcntl(fdLog, F_GETFL); //get all file status flags of fdLog
        flags = flags | O_EXCL; //add O_EXCL to flags
        fcntl(fdLog, F_SETFL, flags);//set flags for the file
        int len = sprintf(buf, "\nprocessMonitor: status:%d The child pid:%d has finished\n", status, pid);//write message to buf
        write(fdLog, buf, len);//write buf to fdLog
        bool isFound = false;
        for (int i = 0; i < NUM && isParent && !isFound; ++i)
        {
            if (pid == childPid[i])
            {
                ...
                //answer: set new flags to the file and close it
                ...
                { //the child
                    ...
                    //answer: because the whole function is for trying to respawn the process if it gets terminated
                    execlp(args[i], args[i], (const char *)key, NULL);
                    cout << strerror(errno) << endl;
                }
            }
