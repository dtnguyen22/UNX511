...
int main(int argc, char *argv[])
{
    ...
    fd = open(keyFile, O_RDWR); //key file /dev/keygen is stored in header file
    //Question 13: This appears to be reading a file under /dev. What is it really doing?
    //answer: it reads data from kernel module using fd and stores data to key2
    read(fd, key2, MAXBUF);
    close(fd);
    ...
    else
    {
        ...
        //answer: sigHandler will catch the signal, memMonitor process will be terminated, however, the processMon will spawn new memMonitor prcoess.
        //Question 15: What happens if memMonitor receives a ctrl-C?
        //answer: sigHandler will catch the signal and isRunning will be set to false, however that does not stop the processMon write to midterm.log
        //, because the processMon will spawn new memMonitor process.
        ...
        //answer: open for reading and writing, create if the file does not exist, writes always append to the end of the file
        int openFlags = O_RDWR | O_CREAT | O_APPEND;
        //Question 17: Describe these file permissions
        //answer: 666, user can read and write, group can read and write, other can read and write.
